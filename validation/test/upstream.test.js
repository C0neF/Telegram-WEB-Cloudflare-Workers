...
test('validation is anchored to the pinned server protocol and Desktop v7.1.2 commits', () => {
  ...
  const protocol = readPinnedSource('server', 'PROTOCOL.md');
  ...
  const frameHeader = readPinnedSource(
    'desktop',
...
test('pinned Desktop source maps 16-byte secret to abridged and negative DC to media', () => {
  const connection = readPinnedSource(
    'desktop',
  ...
  const session = readPinnedSource(
    'desktop',
